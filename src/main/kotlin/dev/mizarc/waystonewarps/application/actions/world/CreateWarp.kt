...
class CreateWarp(private val warpRepository: WarpRepository,
    ...
    fun execute(playerId: UUID, name: String, position3D: Position3D, worldId: UUID,
        ...
        // Auto-assign default group if set
        val defaultGroupId = configService.getDefaultGroupId()
        if (defaultGroupId != null) {
            val groupUuid = runCatching { UUID.fromString(defaultGroupId) }.getOrNull()
            if (groupUuid != null && warpGroupRepository.getById(groupUuid) != null) {
                newWarp.groupId = groupUuid
            }
        }
